...
int main(int argc, char *argv[])
{
    ...
    if (pid < 0) {
        ...
        erro_exec(exec_args[0]); // Chama erro se execvp falhar
        free(exec_args); // Nunca alcançado, mas mantido para clareza
        free(usage_params);
        exit(EXIT_FAILURE);
    } else { // Processo PAI
        int status;
        ...
        // Verificar falha imediata do filho
        for (int i = 0; i < 50; ++i) { // 50 iterações = 500ms
            ...
            usleep(10000); // 10ms por iteração
        }

        free(exec_args);

        // Criar threads de monitoramento
